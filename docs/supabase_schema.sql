...
-- -------------------------------------------------- Encounter 内的聊天
-- 会话只能挂在已建立的 encounter 上：没有双向确认就没有聊天通道。
create table if not exists public.chat_messages (
  message_id   text primary key,
  encounter_id text not null references public.encounters(encounter_id),
  sender_id    text not null,
  text         text not null,
  created_at   timestamptz not null default now()
);
create index if not exists chat_thread_idx
  on public.chat_messages (encounter_id, created_at);
